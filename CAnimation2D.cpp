...
CAnimator2D * CAnimation2D::GetAnimator(CRString _Key)
{
	return m_Animators[_Key];
}
...
void CAnimation2D::SetHandleAnimator(const string & _Key)
{
	...
	if (find != m_Animators.end())
	{
		...
		m_pHandleAnimator->m_pTimer->Reset();
		m_pHandleAnimator->m_iPlayCount = 0;
		m_pHandleAnimator->SetEnable(true);
		m_pHandleAnimator->m_iCurFrame = 0;
		m_pHandleAnimator = m_Animators[_Key];
	}
}

void CAnimation2D::AddAnimator(const string & _Key, const string & _resKey, float _fDelay, bool _bLoop, int _iReturnFrame)
{
	auto find = m_Animators.find(_Key);
	...
	if (find == m_Animators.end())
	{
		...
		pAnimator->Init(_resKey, _fDelay, _bLoop,_iReturnFrame);

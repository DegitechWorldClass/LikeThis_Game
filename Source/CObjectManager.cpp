...
void CObjectManager::RegisterRenderer2D(CRenderer2D * _pRenderer2D)
{
	...
	m_SortedRenderer2DList[_pRenderer2D->m_SortingLayer].sort([&](CRenderer2D *  pPrev, CRenderer2D *  pNext)->bool { return pPrev->m_iOrderInLayer < pNext->m_iOrderInLayer; });
}
...
void CObjectManager::Update()
{
	...
	for (auto& iter = m_Objects.begin(); iter != m_Objects.end();)
	{
		...
		if ((*iter)->GetObjectTag() == Tag::BoundBullet)
		{
			int a;
		}
